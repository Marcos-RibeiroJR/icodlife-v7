'use client';
// apps/doutor/src/app/empresas/page.tsx
...
export default function EmpresasPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    empresaApi.list(search).then(r => setList(r.data?.data ?? [])).catch(() => setList([])).finally(() => setLoading(false));
  }, [search]);

  useEffect(() => { load(); }, [load]);

  const openNew = () => { setForm({ ...EMPTY }); setEditId(null); setError(''); setInfo(''); setShowForm(true); };
  ...
  const lookup = () => {
    ...
    empresaApi.lookupCnpj(form.cnpj)
      .then(r => {
        const d = r.data;
        setForm(f => ({
          ...f,
  ...
  const submit = () => {
    ...
    const req = editId ? empresaApi.update(editId, payload) : empresaApi.create(payload);
  ...
  const remove = (id: string) => {
    ...
    empresaApi.remove(id).then(() => load());
  };
      ...
      <div className="p-6 md:p-8 max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-slate-500 text-sm">Cadastro de empresas clientes de Medicina do Trabalho.</p>
          </div>
          {!showForm && (
            <button onClick={openNew} className="bg-blue-600 text-white font-bold px-4 py-2.5 rounded-xl hover:bg-blue-700 text-sm">
              + Nova empresa
            </button>
          )}
        </div>
        ...
        {showForm ? (
          ...
          <>
            ...
            {loading ? <p className="text-slate-400 text-sm">Carregando…</p> :
              list.length === 0 ? <p className="text-slate-400 text-sm">Nenhuma empresa cadastrada.</p> :
                ...
                {list.map(c => (
                  <div key={c.id} className="bg-white rounded-xl border border-slate-100 p-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => openEdit(c)} className="text-xs text-blue-600 hover:underline font-medium">Editar</button>
