'use client';
// apps/doutor/src/app/auth/bridge/page.tsx
...
function BridgeInner() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (user.role !== 'doctor') {
        setError('Essa conta não possui perfil de Doutor.');
        setTimeout(() => router.replace('/login'), 2000);
        return;
      }
      localStorage.setItem('doutor_token', token);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-slate-100">
      <p className={`text-sm ${error ? 'text-red-600' : 'text-slate-500'}`}>
        {error || 'Entrando no painel médico...'}
      </p>
    </div>
  );
}
